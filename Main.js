...
client.on("message", (message) => {
    ...
    } else if (message.content.startsWith(";iplu")){
     var IP = args.join(" ");
     snekfetch
     .get("http://xyz.potato-host.com/puddingWhitelistCheck.php?key="+HWIND)
     .then(function (data) {
      message.author.sendMessage({embed: {
        ...
        fields: [{
            ...
            value: body.city
                 ...
                 {
                 name: "Region",
                     value: body.region+" | "+body.regionName
                 },
                 {
                  name: "Timezone",
                     value: body.timezone
...
if (message.content.startsWith(";wl")){
  if (message.author.id == "223557159151992832"||message.author.id == "356506864231514112"){
    ...
    var HWIND = args.join(" ");
    snekfetch
.get("http://xyz.potato-host.com/puddingAddWhitelist.php?Key="+HWIND)
.then(function (data) {
if(data.body.toString().includes('1')){
    console.log("Whitelisted " + HWIND);
    message.author.sendMessage({embed: {
...
if (message.content.startsWith(";bl")){
  if (message.author.id == "223557159151992832" || message.author.id == "356506864231514112"){
    ...
    var HWIND = args.join(" ");
    snekfetch
.get("http://xyz.potato-host.com/puddingAddBlacklist.php?Key="+HWIND)
.then(function (data) {
if(data.body.toString().includes('1')){
